...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'bike_and_scooter_rental.settings')
django.setup()
...
def pop_rentals(number):
    for _ in range(number):
        random_rental_date = fake.date_time_between(start_date='-5y', end_date='now', tzinfo=None),
        random_return_date = fake.date_time_between(start_date='-5y', end_date='now', tzinfo=None),
        while random_rental_date > random_return_date:
            random_rental_date = fake.date_time_between(start_date='-5y', end_date='now', tzinfo=None),
            random_return_date = fake.date_time_between(start_date='-5y', end_date='now', tzinfo=None),

        Rental.objects.create(
        rental_date = random_rental_date,
        return_date = random_rental_date,
        customer = Customer.objects.get(id = random.randint(1, 41)),
        vehicle = Vehicle.objects.get(id = random.randint(1,41)),
        )
